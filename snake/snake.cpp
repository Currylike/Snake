 ...
 LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
 {
   ...
   case WM_KEYDOWN:
    ...
    break;
     ...
     SetBkMode(hdc,TRANSPARENT);                //background attribute
     SetTextColor(hdc,RGB(250,2,2));               //text color
     hBrush = CreateSolidBrush(RGB(0,0,200));
     SelectObject(hdc,hBrush);
     
     TextOut(hdc,650,60,szBuff[0],wsprintf(szBuff[0],TEXT ("%s"),TEXT ("function:")));
     TextOut(hdc,640,80,szBuff[1],wsprintf(szBuff[1],TEXT ("%s"),TEXT ("ESC to pause")));
     TextOut(hdc,640,100,szBuff[2],wsprintf(szBuff[2],TEXT ("%s"),TEXT ("R to resume")));
     TextOut(hdc,630,120,szBuff[3],wsprintf(szBuff[3],TEXT ("%s"),TEXT ("Enter to revive")));
     TextOut(hdc,635,200,szBuff[4],wsprintf(szBuff[4],TEXT ("%s"),TEXT ("direction key")));
     TextOut(hdc,640,220,szBuff[5],wsprintf(szBuff[5],TEXT ("%s"),TEXT ("↑↓←→")));
     TextOut(hdc,635,400,szBuff[7],wsprintf(szBuff[7],TEXT ("%s  %d"),TEXT ("Score :"),snakeLen * RECTDISTANCE-40));
     TextOut(hdc,640,480,szBuff[2],wsprintf(szBuff[2],TEXT ("%s"),TEXT ("By :Bug")));
 
      for (k=0;k<DRAFRAMENUM;k++)                 //drawn framework
      {
       Rectangle(hdc,xLeft,yTop+(k*RECTDISTANCE),xRight,yBottom+(k*RECTDISTANCE));
      }
      for (k=0;k<DRAFRAMENUM;k++)
      {
       Rectangle(hdc,xLeft+(k*RECTDISTANCE),yTop,xRight+(k*RECTDISTANCE),yBottom);
      }
     ...
     if(!Pause)                     //drawn snake
     {
      ...
      {            
       Rectangle(hdc,bodyPos[i].xLeft,bodyPos[i].yTop,bodyPos[i].xRight,bodyPos[i].yBottom);
      }
      DeleteObject(hBrush);
      ...
      Rectangle(hdc,beanPos.xLeft,beanPos.yTop,beanPos.xRight,beanPos.yBottom);   //drawn bean
      DeleteObject(hBrush);
      ...
      bean();
		...
		TextOut(hdc,200,230,szBuff [9],wsprintf(szBuff[9],TEXT ("%s"),TEXT ("Pause")));
		DeleteObject(hFont);
	 }
     if(gameOver())
     {
      hFont = CreateFontIndirect(&lf);
      SelectObject(hdc,hFont);
      TextOut(hdc,200,180,szBuff [6],wsprintf(szBuff[6],TEXT ("%s"),TEXT ("Game")));
      TextOut(hdc,200,250,szBuff [7],wsprintf(szBuff[7],TEXT ("%s"),TEXT ("Over")));
      KillTimer(hwnd,ID_TIME);
      over = 1;
      DeleteObject(hFont);
     }
